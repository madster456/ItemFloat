	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		 if (!(sender instanceof Player)) {
	        ...
	        if (!p.hasPermission("if.staff")) {
	            p.sendMessage(ChatColor.GOLD + "[IF] " + ChatColor.RED + "You do not have permission to do this.");
	            return true;
	        }

	        if (p.getInventory().getItemInMainHand().getType() == Material.AIR) {
	            p.sendMessage(p.getInventory().getHeldItemSlot() + "");
	            p.sendMessage(ChatColor.GOLD + "[IF] " + ChatColor.BLUE + "You can not place air!");
	            return true;
	        }
	        ...
	        lore.add("FLOATING");
	        itemmeta.setDisplayName(ChatColor.GOLD + "[IF] " + ChatColor.WHITE + getItemName(item));
	        itemmeta.setLore(lore);
	        item.setItemMeta(itemmeta);
	        p.sendMessage(ChatColor.GOLD + "[IF] " + ChatColor.GREEN + "Drop the item now!");
